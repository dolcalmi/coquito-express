...
function verifyAndCreateResetToken( user ) {
    ...
    jwtConfig.expiresIn = this.config.APP.resetPasswordTokenExpiresIn;

    return Promise.promisify( jwt.sign )( { resetPasswordUserId: user.id }, this.config.APP.secretOrKey, jwtConfig )
        .then(function (token) {
